...
class QrProfilePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final payload = {
      'v': 1, // versão do payload
      'type': 'hiperdiario.profile',
      'name': p.name,
      'cpf': p.cpf,
      ...
      'contact': p.contact,
      'ubs': p.ubs,
      // Opcional: incluir um timestamp de geração
      'generatedAt': DateTime.now().toIso8601String(),
    };

    final json = _minifiedJson(payload);

    return Scaffold(
      appBar: AppBar(title: const Text('QR do Perfil')),
      body: Padding(
        padding: const EdgeInsets.all(16.0),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.center,
          children: [
            const SizedBox(height: 12),
            Text(
              'Mostre este QR Code ao profissional de saúde para leitura segura das suas informações básicas.',
  ...
  static String _minifiedJson(Map<String, dynamic> map) {
    // conversão leve, sem pretty print, para diminuir o QR
  ...
  Widget _privacyNote() => Container(
    ...
    child: Row(
      ...
      children: [
        ...
        const Expanded(
          child: Text(
            'Atenção: este QR contém dados pessoais (LGPD). Compartilhe apenas com profissionais de saúde de confiança.',
          ),
        ),
      ],
    ),
...
class _QrWidget extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Import deferido local para evitar problemas de análises em plataformas sem suporte
    return _QrInner(data: data);
  }
}

class _QrInner extends StatelessWidget {
  final String data;
  const _QrInner({required this.data});
